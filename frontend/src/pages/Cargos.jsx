...
function Cargos() {
  const { role } = useRole();
  console.log("Rol actual del usuario en Cargos:", role);
  ...
  useEffect(() => {
    const fetchCargos = async () => {
      try {
        const response = await axios.get("https://localhost:4000/api/cargos");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (cargoSeleccionado) {
        await axios.put(
          `https://localhost:4000/api/cargos/${cargoSeleccionado._id}`,
          formData
        );
      } else {
        await axios.post("https://localhost:4000/api/cargos", formData);
      }
